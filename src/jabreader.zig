...
pub export fn printUsage() void {
    _ = printf("\n");
    _ = printf("jabcodeReader (Version %s Build date: %s) - Fraunhofer SIT\n\n", "2.0.0", "May 24 2025");
    _ = printf("Usage:\n\n");
    _ = printf("jabcodeReader input-image(png) [--output output-file]\n");
    _ = printf("\n");
    _ = printf("--output\tOutput file for decoded data.\n");
...
pub export fn main(arg_argc: c_int, arg_argv: [*c][*c]u8) c_int {
    ...
    var output_as_file: jab_boolean = 0;
    _ = &output_as_file;
    if (argc > @as(c_int, 2)) {
        if (@as(c_int, 0) == strcmp(argv[@as(c_uint, @intCast(@as(c_int, 2)))], "--output")) {
            output_as_file = 1;
        } else {
            _ = printf("Unknown parameter: %s\n", argv[@as(c_uint, @intCast(@as(c_int, 2)))]);
            return 255;
        }
    }
    var bitmap: [*c]jab_bitmap = undefined;
    _ = &bitmap;
    bitmap = readImage(argv[@as(c_uint, @intCast(@as(c_int, 1)))]);
    if (bitmap == @as([*c]jab_bitmap, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return 255;
    var decode_status: jab_int32 = undefined;
    _ = &decode_status;
    var symbols: [61]jab_decoded_symbol = undefined;
    _ = &symbols;
    var decoded_data: [*c]jab_data = decodeJABCodeEx(bitmap, @as(c_int, 0), &decode_status, @as([*c]jab_decoded_symbol, @ptrCast(@alignCast(&symbols[@as(usize, @intCast(0))]))), @as(c_int, 61));
    _ = &decoded_data;
    if (decoded_data == @as([*c]jab_data, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        free(@as(?*anyopaque, @ptrCast(bitmap)));
        reportError(@as([*c]u8, @ptrCast(@volatileCast(@constCast("Decoding JABCode failed")))));
        if (decode_status > @as(c_int, 0)) return @as(jab_int32, @intFromFloat(symbols[@as(c_uint, @intCast(@as(c_int, 0)))].module_size + 0.5)) else return 255;
    }
    if (decode_status == @as(c_int, 2)) {
        {
            _ = printf("JABCode Info: ");
            _ = printf("The code is only partly decoded. Some slave symbols have not been decoded and are ignored.");
            _ = printf("\n");
        }
    }
    if (output_as_file != 0) {
        var output_file: [*c]FILE = fopen(argv[@as(c_uint, @intCast(@as(c_int, 3)))], "wb");
        _ = &output_file;
        if (output_file == @as([*c]FILE, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            reportError(@as([*c]u8, @ptrCast(@volatileCast(@constCast("Can not open the output file")))));
            return 255;
        }
        _ = fwrite(@as(?*const anyopaque, @ptrCast(decoded_data.*.data())), @as(c_ulong, @bitCast(@as(c_long, decoded_data.*.length))), @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), output_file);
        _ = fclose(output_file);
    } else {
        {
            var i: jab_int32 = 0;
            _ = &i;
            while (i < decoded_data.*.length) : (i += 1) {
                _ = printf("%c", @as(c_int, @bitCast(@as(c_uint, decoded_data.*.data()[@as(c_uint, @intCast(i))]))));
            }
        }
        _ = printf("\n");
    }
    free(@as(?*anyopaque, @ptrCast(bitmap)));
    free(@as(?*anyopaque, @ptrCast(decoded_data)));
    return 0;
}
